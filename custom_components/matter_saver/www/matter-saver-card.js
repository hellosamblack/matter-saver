class MatterSaverCard extends HTMLElement {
  ...
  _getDevices(state) {
    ...
    const normalized = devices.map((device) => this._normalizeDevice(device));
    const byId = new Map(normalized.map((device) => [device.node_id, device]));

    devices.forEach((device, index) => {
      if (device && !("node_id" in device) && Array.isArray(device.rt)) {
        normalized[index].route_path = device.rt.map((hop) => this._normalizeRouteHop(hop, byId));
      }
    });
  ...
  _normalizePower(power) {
    const powers = { b: "battery", w: "wired", u: "unknown" };
    return powers[power] || power || "unknown";
  }
